...
const ChatItem = ({
    ...
    const makeRequest = async () => {
      const filteredEvent = events?.filter(
        (event: { invite_chat_link: any }) =>
          event.invite_chat_link === chat.invite_chat_link
      )
  ...
  const toggleActivetiy = (chat: {
    enabled: any
    _id: string
  }) => {
    if (!chat.enabled) {
      apis.startEvent(chat._id)
      // setActiveEvent({ ...chat, enabled: true })
    } else {
      apis.stopEvent(chat._id)
      // setActiveEvent({ ...chat, enabled: false })
    }
  }
      ...
      <div className={"bots-item"}>
        <div className={"w-100"}>
          ...
          >
            <h4
              ...
              data-tooltip-content={`${chat?.title}`}
              data-tooltip-place="bottom"
            >
              {chat.title || "Название отсутствует"}
            </h4>
            ...
            <div className="form-check form-switch form-switch-lg">
              <input
                ...
                defaultChecked={chat.enabled}
                data-tooltip-id="my-tooltip"
                data-tooltip-content={`Отключить все рассылки`}
                data-tooltip-place="bottom"
                onClick={() => toggleActivetiy(chat)}
              />
            </div>
          </div>
          ...
          >
            <a href={`${chat.invite_chat_link}`} target="blank">
              <img
        ...
        {activeEvents && activeEvents.length >= 1 ? (
          <div className={"w-100"}>
            ...
            <Link
              to={`/chats/${chat._id}/${filteredEvents[0]?._id}`}
        ...
        <div className={"w-100"}>
          <Link
            to={`/events-settings/${chat.chat_id}`}
