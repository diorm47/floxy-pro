...
import "./Chats.scss"
import ChatsSettingsItem from "./ChatSettingsItem"
...
const ChatSettings: FC = () => {
  ...
  React.useEffect(() => {
    document.title = `${activeChat?.title} - Чаты | Floxy.pro`
  }, [activeChat?.title])
  ...
  useEffect(() => {
    const makeRequest = async () => {
      const activeChatData = await apis.getEvent(chatId)
      setactiveChat(activeChatData)

      const eventsData = await apis.getEvents()
      const filteredEvents = eventsData?.filter(
        (event: { param_id: any }) =>
          event.invite_chat_link === activeChatData?.invite_chat_link
      )
      ...
      //   (event: { invite_chat_link: any }) => {
      //     return event.invite_chat_link === activeChat.invite_chat_link
      //   }
      // )[0]
  ...
  const onSubmit: SubmitHandler<IEvent> = async data => {
    ...
    if (selectedBots) {
      bots = selectedBots.map((bot: { value: any }) => ({
        // bot_id: "63eff55c68fc13bf8441a884",
        bot_id: bot.value,
        join_chat: true,
      }))
    }
    ...
    const formattedData = {
      ...
      // param_id: activeChat._id,
      event_name: "join_chat",
      // user_id_path: customUserIdPath || data.selectedFilter.value,
      send_many_times: sendManyTimes,
      filter_fields: filterField,
      bots: bots,
      // invite_chat_link: activeLink || activeEvent.invite_chat_link,
      check_user_in_chat: checkUserInChat,
      messages: {
    ...
    const filteredEvents = eventsData?.filter(
      (event: { param_id: any }) =>
        event.param_id === activeChatData?.chat_id
    )
    setEvents(filteredEvents)
  ...
  const onDeleteBtnClick = async () => {
    ...
    const testFunc = async () => {
      const activeChatData = await apis.getGroup(Number(chatId))
      setactiveChat(activeChatData)
      const eventsData = await apis.getEvents()
      const filteredEvents = eventsData?.filter(
        (event: { param_id: any }) =>
      ...
      await navigate(
        `/groups/${activeChat.chat_id}/${filteredEvents[0]._id}`
      )
    }
    await testFunc()
    setIsCreatingItem(false)
    ...
    const activeChatData = await apis.getGroup(Number(currentchatId))
    setactiveChat(activeChatData)
  ...
  const onItemClick = (
    ...
    navigate(`/chats/${eventId}/${event?._id}`)
  }
            ...
            >
              <div>
                ...
                <p className="text_margin">
                  Вы или ваш бот обязательно должны быть
              ...
              <div>
                <CardTitle className="mb-2">
                  Проверять есть ли пользователь в чате в момент
                  отправки:
                </CardTitle>
                <div className="form-check form-switch form-switch-lg">
                  {checkUserInChat ? (
                    <div
                      onClick={() =>
                        setCheckUserInChat(!checkUserInChat)
                      }
                    >
                      <input
                        type="checkbox"
